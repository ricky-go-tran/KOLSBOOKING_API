class Api::V1::Admin::JobsController < Api::V1::Admin::BaseController
  ...
  def index
    ...
    tab = params[:tab]
    jobs = Job.includes(:profile, image_attachment: :blob).order(created_at: :desc)
    jobs = jobs.search_by_title(search) if search.present?
    jobs = jobs.where(status: params[:tab]).order(created_at: :desc) if tab.present? && JOBS_ACCEPTED_PARAMS.include?(tab)
